...
public class SmtpPanelUiTests
{
    ...
    public async Task SaveEmailSettings_PostsToDedicatedSmtpEndpoint()
    {
        ...
        // #389: the SMTP save now requires a host + from-address client-side, so seed
        // both — a blank config is (correctly) blocked and covered by FormValidationTests.
        using var ctx = Ctx(new SiteSettingsViewModel { SmtpHost = "mail.test", SmtpFromAddress = "alerts@test" }, posts);
        var provider = await Open(ctx);
