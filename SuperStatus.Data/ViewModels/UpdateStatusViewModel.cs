namespace SuperStatus.Data.ViewModels
{
    ...
    public class UpdateStatusViewModel
    {
        ...
        /// <summary>Issue #311: whether the console can apply an update in-app via the
        /// "Update now" button. True only when Watchtower's http-api trigger is
        /// configured (URL + token present on the api service). The app still never
        /// touches the Docker socket — it calls Watchtower's HTTP API. When false the
        /// panel falls back to the guided command. Never carries the token.</summary>
