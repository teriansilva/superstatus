...
/// <summary>
/// Issue #293 Phase C — the SLA admin surface on the console's Checks tab:
/// the SlaListPanel (rows + default star, set-default flow, delete verdicts
...
public class SlaAdminPanelTests
{
    ...
    [TestMethod]
    public void ListPanel_RendersRows_DefaultStarred_NoSetDefaultOnTheDefault()
    {
        ...
        Assert.IsNotNull(rows[0].QuerySelector(".sla-star"), "the default row is starred");
        Assert.IsNull(rows[1].QuerySelector(".sla-star"));

        Assert.AreEqual(0, rows[0].QuerySelectorAll("button").Count(b => b.TextContent.Trim() == "Set default"),
            "the default row offers no Set default");
        Assert.AreEqual(1, rows[1].QuerySelectorAll("button").Count(b => b.TextContent.Trim() == "Set default"));
        Assert.IsTrue(cut.Markup.Contains("Exactly one default at all times"), "the explanatory footnote");
    }
    ...
    public void SetDefault_PatchesTheApi_AndReloads()
    {
        ...
        cut.FindAll(".sla-link").First(b => b.TextContent.Trim() == "Set default").Click();
    ...
    public void Delete_LinkedSla_RendersSharedBlockedDialog_FromThe409Payload()
    {
        ...
        // Row 2 (Messenger 95) is linked → straight to the API; the 409
        // LinkedEntitySummary drives the SHARED blocked dialog.
        cut.FindAll(".sla-link.danger")[1].Click();
    ...
    public void Delete_DefaultSla_SurfacesThe422Message()
    {
        ...
        cut.FindAll(".sla-link.danger")[0].Click();
    ...
    public void Delete_UnlinkedNonDefault_ConfirmsThenDeletes()
    {
        ...
        cut.FindAll(".sla-link.danger")[2].Click();
    ...
    public void EditDialog_PlainLanguageRows_AndLivePreview_RelaxedSlaIsGreen()
    {
        ...
        cut.FindAll(".sla-link").First(b => b.TextContent.Trim() == "Edit" &&
            b.Closest(".sla-row")!.TextContent.Contains("Messenger 95")).Click();
    ...
    public void EditDialog_LivePreview_StrictDefaultIsRed_AndRecomputesOnInput()
    {
        ...
        cut.FindAll(".sla-link").First(b => b.TextContent.Trim() == "Edit" &&
            b.Closest(".sla-row")!.TextContent.Contains("Default")).Click();
