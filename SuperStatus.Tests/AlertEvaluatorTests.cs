...
/// #343 Phase 1: delivery now routes through the notification-provider registry —
/// the stub notifiers are wrapped by the real Email/WebPush providers, so these
/// same assertions exercise the full seam and prove behavior parity.
/// </summary>
[TestClass]
public class AlertEvaluatorTests
{
    // Wraps the stub notifiers in the real channel providers behind the registry, so
    // the existing assertions double as the seam's parity guard.
    private static (AlertEvaluator eval, SuperStatusDb db, SqliteConnection conn) Build(
        ...
        => BuildWith(new NotificationProviderRegistry(new INotificationProvider[]
        {
            new EmailNotificationProvider(notifier ?? new StubEmailNotifier()),
            new WebPushNotificationProvider(webPush ?? new StubWebPushNotifier()),
        }));

    private static (AlertEvaluator eval, SuperStatusDb db, SqliteConnection conn) BuildWith(
    ...
    /// <summary>A channel provider that always throws — proves the evaluator contains a
    /// misbehaving channel. Uses the email TypeId so an email-enabled check routes to it.</summary>
    private sealed class ThrowingProvider : INotificationProvider
    {
        public NotificationDescriptor Descriptor { get; } =
            new(EmailNotificationProvider.TypeId, "Throwing", "x", supportsTest: false);
    ...
    public async Task WebPushFire_sends_recordsFiredWithDeviceTarget()
    {
        ...
        var row = await db.AlertDeliveryLogSet.SingleAsync();
        Assert.AreEqual(NotificationChannelTypes.WebPush, row.ChannelTypeId);
        Assert.AreEqual(AlertOutcome.Fired, row.Outcome);
    ...
    [TestMethod]
    public async Task UnknownChannelProvider_isContained_logsSkipped_noThrow()
    {
        // #343 Phase 1 containment: an enabled channel whose TypeId has no registered
        // provider must not crash the tick — it logs one calm Skipped row.
        var (eval, db, conn) = BuildWith(new NotificationProviderRegistry(Array.Empty<INotificationProvider>()));
        using var _ = db; using var __ = conn;
        var check = AddCheck(db); // email-enabled → routes to the "email" TypeId

        check.ConsecutiveFailures = 2; check.DownSinceUtc = DateTime.UtcNow.AddSeconds(-30);
        await db.SaveChangesAsync();
        await eval.EvaluateAsync(check, FailType.StatusCode); // must not throw

        var row = await db.AlertDeliveryLogSet.SingleAsync();
        Assert.AreEqual(AlertOutcome.Skipped, row.Outcome, "an unregistered channel is a calm skip");
        Assert.AreEqual("channel provider not registered", row.Reason);
        Assert.IsNull(row.ErrorMessage);
    }

    [TestMethod]
    public async Task ChannelProviderThrows_isContained_logsFailed_noThrow()
    {
        // #343 Phase 1 containment: a channel provider that throws is converted to a
        // Failed audit row and never disrupts the tick — probe-seam parity.
        var (eval, db, conn) = BuildWith(new NotificationProviderRegistry(
            new INotificationProvider[] { new ThrowingProvider() }));
        using var _ = db; using var __ = conn;
        var check = AddCheck(db); // email-enabled → routes to the throwing provider
        ...
        var row = await db.AlertDeliveryLogSet.SingleAsync();
        Assert.AreEqual(AlertOutcome.Failed, row.Outcome, "a throwing channel is a Failed row, not a crash");
        Assert.AreEqual("channel error", row.ErrorMessage);
    }
    ...
    public async Task Fires_once_perOutageEpisode_thenRecovers()
    {
        ...
        var row = await db.AlertDeliveryLogSet.SingleAsync();
        Assert.AreEqual(NotificationChannelTypes.Email, row.ChannelTypeId);
    ...
    public async Task ChannelsDisabledThenHealthy_clearsStaleMarker()
    {
        ...
        // Channel disabled (#343 Phase 3: on the profile's channel collection), then the
        // check recovers — the stale marker must still be cleared so re-enabling a
        // channel later can't replay an old recovery.
        db.AlertProfileChannelSet.Single(c => c.ProviderType == NotificationChannelTypes.Email).IsEnabled = false;
        check.ConsecutiveFailures = 0; check.DownSinceUtc = null;
        await db.SaveChangesAsync();
        await eval.EvaluateAsync(check, FailType.NoFail);
    ...
    public async Task BothChannels_fireTogether_oneRowEach()
    {
        ...
        Assert.AreEqual(2, await RowCount(db));
        var channels = await db.AlertDeliveryLogSet.Select(x => x.ChannelTypeId).OrderBy(x => x).ToListAsync();
        CollectionAssert.AreEqual(new[] { NotificationChannelTypes.Email, NotificationChannelTypes.WebPush }, channels);
    }
