...
public class SlaPhaseBTests
{
    ...
    private static HistoricalStatusData Tick(long checkId, DateTime when, int code = 200, long ms = 50) => new()
    {
        ...
        // #418: rollup classifies from FailType, not the HTTP status. Down via code 500 → StatusCode; slow (ms) is latency-driven.
        FailType = code >= 200 && code < 400 ? FailType.NoFail : FailType.StatusCode,
    };
