...
public enum UpdateTriggerOutcome
{
    ...
    Accepted,
    /// <summary>No update engine on this install (no Watchtower http-api URL/token) —
    /// the operator opted out with --no-updater / SUPERSTATUS_UPDATE_ENGINE=none.</summary>
    NotConfigured,
...
/// it asks Watchtower (the one container that holds the socket) to pull + restart via
/// its authenticated HTTP API. <see cref="CanApply"/> reflects whether that path is
/// configured, so the UI only offers the button when it will work — which, since #334,
/// is every default install.
/// </summary>
public interface IUpdateTrigger
{
    /// <summary>True when the Watchtower http-api trigger is configured (URL + token) —
    /// i.e. the update engine is present. False only on an opted-out install
    /// (SUPERSTATUS_UPDATE_ENGINE=none), where the console shows the guided command.</summary>
    bool CanApply { get; }
...
/// environment (layered on by docker-compose.watchtower.yml, which the default install
/// includes). The token is kept here and in the outbound Authorization header only —
/// never serialized to a view model, sent to the browser, or logged.
/// </summary>
    ...
    public async Task<UpdateTriggerResult> TriggerAsync(CancellationToken cancellationToken = default)
    {
        if (!CanApply)
            return new UpdateTriggerResult(UpdateTriggerOutcome.NotConfigured,
                "This install has no update engine (--no-updater). Re-run install.sh without it, or use the command below.");
