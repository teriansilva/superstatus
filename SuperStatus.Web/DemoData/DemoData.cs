...
public static class DemoData
{
    ...
    public static SiteSettingsViewModel Settings() => new()
    {
        ...
        SmtpFromName = "Acme Status",
        SmtpPasswordSet = true,
        AlertDefaultRecipients = "ops@example.com, oncall@example.com",
        SmtpVerifiedUtc = new DateTime(2026, 5, 29, 8, 30, 0, DateTimeKind.Utc),
    };

    public static List<ProviderDescriptorViewModel> CheckProviders() =>
    [
        new()
        {
            TypeId = "http",
            ...
            [
                ...
                new() { Key = "expectedStatusCode", Label = "Expected status", Kind = "number", Required = true },
            ],
        },
        new()
        {
            ...
            Icon = "sparkle",
            SchemaVersion = 1,
            Description = "Pull probe — streams a canary prompt to an OpenAI-compatible endpoint.",
            Direction = "pull",
            BatchTargetField = "baseUrl",
            // #433: mirrors AiCheckProvider's real schema, so the gallery exercises the guided
            // flow's actual shape — a reduced copy would let the staged reveal look fine here
            // while the deferred fields it hides don't exist.
            Fields =
            [
                new() { Key = "baseUrl", Label = "Base URL", Kind = "text", Required = true, Placeholder = "https://api.openai.com/v1" },
                new() { Key = "model", Label = "Model", Kind = "text", Required = true, Placeholder = "gpt-4o-mini" },
                new() { Key = "apiKey", Label = "API key", Kind = "secret", Required = false },
                new() { Key = "prompt", Label = "Canary prompt", Kind = "text", Required = true, Placeholder = "Reply with the single word: pong" },
                new() { Key = "expectContains", Label = "Response must contain", Kind = "text", Required = true, Placeholder = "pong" },
                new() { Key = "maxTokens", Label = "Max tokens", Kind = "number", Required = false, Placeholder = "32" },
                new() { Key = "ttftThresholdMs", Label = "TTFT threshold (ms)", Kind = "number", Required = false },
                new() { Key = "minTokensPerSec", Label = "Min tokens/sec", Kind = "number", Required = false },
            ],
            Metrics =
            [
                new() { Key = "ttft_ms", Label = "TTFT", Unit = "ms", Kind = "gauge" },
                new() { Key = "tokens_per_sec", Label = "Throughput", Unit = "tok/s", Kind = "gauge" },
            ],
        },
        new()
        {
            ...
            Icon = "pulse",
            SchemaVersion = 1,
            Description = "Push / dead-man's-switch — the agent pings a token URL each run.",
            Direction = "push",
            Fields =
            [
                new() { Key = "intervalSeconds", Label = "Expected interval (s)", Kind = "number", Required = true },
                new() { Key = "graceSeconds", Label = "Grace (s)", Kind = "number", Required = true },
            ],
            Metrics =
            [
                new() { Key = "seconds_since_heartbeat", Label = "Since heartbeat", Unit = "s", Kind = "gauge" },
            ],
        },
    ];

    /// <summary>#433: canned model list for the guided AI setup. Demo mode must never let the
    /// dialog's Validate button reach a real endpoint — that would turn the public demo into an
    /// open outbound relay — so discovery is answered from here.</summary>
    public static AiModelsResultViewModel AiModels() => new()
    {
        Ok = true,
        SupportsListing = true,
        Models = ["gpt-4.1", "gpt-4o-mini", "llama-3.3-70b-instruct", "mistral-large-latest"],
        Message = "Connected — 4 models.",
    };

    /// <summary>#433: canned dry-run verdict, so the demo shows the Test connection affordance
    /// working without any outbound request. The AI provider gets its declared metrics so the
    /// verdict strip renders its full shape.</summary>
    public static CheckTestResultViewModel CheckTestResult(string providerType) => providerType == "ai"
        ? new CheckTestResultViewModel
        {
            Outcome = "up",
            Ok = true,
            LatencyMs = 988,
            ...
            Metrics =
            [
                new() { Key = "ttft_ms", Label = "TTFT", Unit = "ms", Value = 412 },
                new() { Key = "tokens_per_sec", Label = "Throughput", Unit = "tok/s", Value = 63.4 },
            ],
        }
        : new CheckTestResultViewModel
        {
    ...
    public static List<NotificationDescriptorViewModel> NotificationProviders() =>
    [
        new()
        {
            TypeId = NotificationChannelTypes.Email,
        ...
        new()
        {
            TypeId = NotificationChannelTypes.WebPush,
        ...
        new()
        {
            ...
            Fields =
            [
                new() { Key = "webhookUrl", Label = "Webhook URL", Kind = "secret", Required = true },
            ],
        },
    ];
        ...
        return new Dictionary<DateTime, List<IncidentViewModel>>
        {
            ...
            [
                new IncidentViewModel
                {
                    ...
                    Description = "**P95 latency above the 1s window** for two consecutive ticks. Upstream provider reporting slow response times.\n\n- Routing new requests to the `fallback` region\n- Next update in 15 min\n\nTracking on the [provider status page](https://status.provider.example.com).",
