...
public class WebhookChannelTests
{
    ...
    {
        var json = new WebhookChannelSettings("https://hook.example/fire").ToJson();
    ...
    private static NotificationContext Ctx(string? url, AlertTrigger trigger = AlertTrigger.Failure)
    {
        var check = new StatusCheck { Title = "Public API", StatusCheckUrl = "https://api/health", ServiceLogoUrl = "", ConsecutiveFailures = 3 };
        var configJson = url is null ? null : new WebhookChannelSettings(url).ToJson();
