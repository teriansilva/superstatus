...
namespace SuperStatus.Data.DatabaseContext
{
    public class SuperStatusDb(DbContextOptions<SuperStatusDb> options) : DbContext(options)
    {
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // #343 Phase 5: the delivery channel is a stable type-id string (was the
            // AlertChannel enum column) so arbitrary channels log correctly.
            modelBuilder.Entity<AlertDeliveryLog>()
             .Property(x => x.ChannelTypeId)
             .IsRequired();
            ...
            // Issue #334: automatic APPLY defaults OFF — the read-only check above is
            // safe to leave on, an unattended pull+restart is not. The daily time has a
            // database default so the column added on upgrade lands at 03:00 UTC rather
            // than midnight (the implicit TimeOnly default).
            modelBuilder.Entity<SiteSettings>()
             .Property(s => s.AutoUpdateEnabled)
             .HasDefaultValue(false);

            modelBuilder.Entity<SiteSettings>()
             .Property(s => s.AutoUpdateTimeUtc)
             .HasDefaultValue(new TimeOnly(3, 0));
            ...
            // #415: SourceStatusCheckId is a real (nullable) FK to the source check
            // with ON DELETE CASCADE. This makes check-deletion vs auto-incident-creation
            // atomic at the DB and closes the delete-during-draft race in
            // AutoIncidentWorker without any app-level locking:
            //   - an insert of an auto-incident for a check that is already gone is
            //     rejected (FK violation);
            //   - an insert that commits just before the check is deleted is removed with
            //     the check by the cascade — so a phantom open incident can never survive.
            // Trade-off (chosen deliberately): a deleted check's incidents — open and
            // historical — are removed with it; operators who want to keep a service's
            // incident record disable the check rather than delete it. No navigation
            // property — the link is only ever traversed as a filter.
            modelBuilder.Entity<Incident>()
             .HasOne<StatusCheck>()
             .WithMany()
             .HasForeignKey(x => x.SourceStatusCheckId)
             .OnDelete(DeleteBehavior.Cascade);
            ...
            // #343 Phase 3: per-profile channel collection (source of truth for delivery).
            // Cascade-deletes with the profile; at most one row per (profile, channel type).
            modelBuilder.Entity<AlertProfileChannel>()
             .HasOne(x => x.AlertProfile)
            ...
            modelBuilder.Entity<AlertProfileChannel>()
             .HasIndex(x => new { x.AlertProfileId, x.ProviderType })
             .IsUnique()
             .HasDatabaseName("IX_AlertProfileChannelSet_AlertProfileId_ProviderType");
            ...
            // #320 Phase 2b: the heartbeat ping token is the endpoint's only credential,
            // so the lookup must be a unique, indexed point read. Partial-unique over the
            // non-null rows — only "heartbeat" checks carry a token; every other provider
            // leaves it null and those nulls must not collide. SQLite honours the same
            // partial-index syntax, so the tests exercise the uniqueness too.
            modelBuilder.Entity<StatusCheck>()
             .HasIndex(x => x.HeartbeatToken)
             .IsUnique()
             .HasFilter("\"HeartbeatToken\" IS NOT NULL")
             .HasDatabaseName("IX_StatusCheckSet_HeartbeatToken");
        ...
        // #343 Phase 3: per-profile delivery channels (source of truth).
        public DbSet<AlertProfileChannel> AlertProfileChannelSet { get; set; }
