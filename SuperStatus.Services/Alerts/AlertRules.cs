    ...
    public static AlertDecision Decide(StatusCheck check, bool isFailure, DateTime nowUtc)
    {
        if (!isFailure)
        {
            ...
            bool firedThisOutage = check.AlertOnRecovery
                && check.AlertedOutageDownSinceUtc is not null
                && check.AlertLastFiredUtc is not null
                && check.AlertLastFiredUtc >= check.AlertedOutageDownSinceUtc;
            if (firedThisOutage)
            ...
            if (check.AlertedOutageDownSinceUtc is not null)
        ...
        if (check.AlertedOutageDownSinceUtc == check.DownSinceUtc)
            return AlertDecision.None;
        ...
        bool throttled = check.AlertLastFiredUtc is not null
            && (nowUtc - check.AlertLastFiredUtc.Value).TotalMinutes < check.AlertThrottleMinutes;
        if (throttled)
