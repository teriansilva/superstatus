...
public class DailyRollupPersistenceTests
{
    ...
    {
        var c = new StatusCheck { Title = "svc", StatusCheckUrl = "x", WebHookOnErrorUrl = "", ServiceLogoUrl = "", ExpectedStatusCode = 200, ExpectedResponseTimeInMs = 1000, Enabled = true, IntervalSeconds = 30, Created = DateTime.UtcNow };
    ...
    public async Task Refresh_PersistsPerDayRollupsFromRaw()
    {
        ...
        db.HistoricalStatusDataSet.AddRange(
            new HistoricalStatusData { StatusCheckId = c.Id, TimeOfCheckUTC = d1, HttpStatusCode = 200, ResponseTimeInMs = 100 },
            new HistoricalStatusData { StatusCheckId = c.Id, TimeOfCheckUTC = d1.AddMinutes(1), HttpStatusCode = 500, ResponseTimeInMs = 100 }); // down
