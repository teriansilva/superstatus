...
public class AlertLogAdminTests
{
    ...
    {
        db.StatusCheckSet.Add(new StatusCheck { Id = 1, Title = "acme", StatusCheckUrl = "x", WebHookOnErrorUrl = "y", ServiceLogoUrl = "z" });
        var t = new DateTime(2026, 6, 8, 10, 0, 0, DateTimeKind.Utc);
        db.AlertDeliveryLogSet.AddRange(
            new AlertDeliveryLog { StatusCheckId = 1, AttemptedUtc = t.AddMinutes(1), Channel = AlertChannel.Email, Trigger = AlertTrigger.Outage, Outcome = AlertOutcome.Fired, Reason = "logged only (no delivery channel wired yet)" },
            new AlertDeliveryLog { StatusCheckId = 1, AttemptedUtc = t.AddMinutes(2), Channel = AlertChannel.Email, Trigger = AlertTrigger.Failure, Outcome = AlertOutcome.Skipped, Reason = "throttled" },
            new AlertDeliveryLog { StatusCheckId = 1, AttemptedUtc = t.AddMinutes(3), Channel = AlertChannel.Email, Trigger = AlertTrigger.Outage, Outcome = AlertOutcome.Failed, ErrorMessage = "smtp refused" });
        db.SaveChanges();
    }
    ...
    public async Task AddOrUpdate_RoundTripsAlertRules_andClampsNegatives()
    {
        ...
        var created = await svc.AddOrUpdateStatusCheck(new StatusCheckViewModelBase
        {
            ...
            EmailAlertsEnabled = true, EmailRecipients = "  ops@example.com ",
            AlertOnFailureThreshold = 3, AlertOnOutageMinutes = 5, AlertOnRecovery = true,
            AlertThrottleMinutes = -7, WebPushAlertsEnabled = true,
        });
        Assert.IsTrue(created.EmailAlertsEnabled);
        Assert.AreEqual("ops@example.com", created.EmailRecipients, "trimmed");
        ...
        Assert.IsTrue(created.WebPushAlertsEnabled);

        // Update toggles things off + changes a threshold.
        var updated = await svc.AddOrUpdateStatusCheck(new StatusCheckViewModelBase(created)
        {
            EmailAlertsEnabled = false, AlertOnFailureThreshold = 1, AlertOnRecovery = false,
        });
        Assert.IsFalse(updated.EmailAlertsEnabled);
