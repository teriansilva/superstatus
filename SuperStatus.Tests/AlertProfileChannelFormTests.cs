...
public class AlertProfileChannelFormTests
{
    ...
    // #365: on a NEW channel the required secret shows its real placeholder (not the
    // "leave blank to keep" trap), and an enabled testable channel offers a Send test button.
    [TestMethod]
    public void NewChannel_SecretField_ShowsRealPlaceholder_AndOffersTestButton()
    {
        using var ctx = Ctx();
        var provider = ctx.RenderComponent<MudDialogProvider>();
        var cut = ctx.RenderComponent<AlertProfileListPanel>();
        ...
        provider.Find(".ap-chan-slack input").Change(true);
        provider.WaitForAssertion(() => Assert.AreEqual(1, provider.FindAll(".ap-cfg-slack-url").Count));

        var placeholder = provider.Find(".ap-cfg-slack-url input").GetAttribute("placeholder");
        StringAssert.Contains(placeholder ?? "", "hooks.slack.com", "a NEW required secret shows the field's real placeholder");
        Assert.AreNotEqual("leave blank to keep", placeholder, "the 'leave blank to keep' trap must not show on a new secret");

        Assert.AreEqual(1, provider.FindAll(".ap-test-slack").Count, "an enabled testable channel offers a Send test button");
    }

    // #365: editing a channel that ALREADY has a stored secret keeps the write-only
    // "leave blank to keep" hint (blank preserves the stored credential).
    [TestMethod]
    public async Task ExistingChannel_WithStoredSecret_ShowsLeaveBlankToKeep()
    {
        using var ctx = Ctx();
        var provider = ctx.RenderComponent<MudDialogProvider>();
        var service = ctx.Services.GetRequiredService<IDialogService>();

        var profile = new AlertProfileViewModel { Id = 9, Name = "Ops" };
        profile.Channels.Add(new AlertProfileChannelViewModel
        {
            ProviderType = "slack",
            IsEnabled = true,
            StoredSecretKeys = { "url" },
        });
        var parameters = new DialogParameters<AlertProfileEditDialog> { { x => x.Profile, profile } };

        await provider.InvokeAsync(() => service.ShowAsync<AlertProfileEditDialog>("edit", parameters));

        provider.WaitForAssertion(() => Assert.AreEqual(1, provider.FindAll(".ap-cfg-slack-url").Count,
            "the stored slack channel renders its secret field"));
        Assert.AreEqual("leave blank to keep", provider.Find(".ap-cfg-slack-url input").GetAttribute("placeholder"),
            "an already-stored secret shows the write-only 'leave blank to keep' hint");
    }
}
