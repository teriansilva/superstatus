...
namespace SuperStatus.Data.Entities
{
    ...
    public class AlertDeliveryLog : EntityBase
    {
        ...
        /// <summary>
        /// #291: the linked alert profile this decision was for. Nullable +
        /// ON DELETE SET NULL so deleting an (unlinked) profile never destroys
        /// audit history; pre-#291 rows are null.
        /// </summary>
        ...
        /// <summary>#343 Phase 5: the delivery channel's stable type id (the provider's
        /// <c>TypeId</c> / <see cref="AlertProfileChannel.ProviderType"/> — e.g. <c>email</c>,
        /// <c>webpush</c>, <c>webhook</c>, <c>slack</c>). Replaces the old fixed
        /// <c>AlertChannel</c> enum column so arbitrary channels log correctly.</summary>
