...
{
    /// <summary>
    /// Issue #358 — true for the no-proxy single-host <b>dynamic issuer</b>
    /// profile, where the browser-facing login redirect + the redirect_uri validation
    /// follow the request host (browser reaches Web and Identity on different published
    /// ports). The issuer itself is pinned to the internal authority (#363) so the
    /// authorize + token endpoints agree on it.
    ///
    /// The discriminator is <c>IDP_PUBLIC_HTTP</c> <b>and</b> <c>WEBAPP_HTTP</c>
    /// both unset. That distinguishes it from the two other shapes: the
    /// <b>reverse-proxy</b> deployment (issuer from the forwarded host) sets
    /// <c>WEBAPP_HTTP</c> to its public URL, and the <b>pinned</b> deployment sets
    /// <c>IDP_PUBLIC_HTTP</c>. Only when neither is present are we on the two-port
    /// self-host stack that needs the same-origin redirect + host-gate handlers.
    /// </summary>
    public static bool IsDynamicSelfHost(string? publicAuthority, string? webAppBaseUrl) =>
        string.IsNullOrWhiteSpace(publicAuthority) && string.IsNullOrWhiteSpace(webAppBaseUrl);
