...
The installer detects the host IP and, on a fresh install, lets you confirm or
change the **advertised host** — the address baked into the login (OIDC) issuer
(`WEBAPP_HTTP` / `IDP_PUBLIC_HTTP`). This matters on a **cloud / NAT'd VM**:
`hostname -I` only sees the private NIC (e.g. `10.0.0.4`), so accepting it would
make sign-in redirect to an address your browser can't reach. Enter the host's
**public IP or a hostname** at the prompt, or set it non-interactively:

```bash
HOST_IP=<public-ip-or-hostname> ./start.sh
```
...
do not receive Docker socket access and still cannot apply updates themselves;
the admin console only reflects that Watchtower is active via
