...
public class AlertProfileChannelConfigTests
{
    ...
    private static string? StoredPayloadJson(AlertProfileChannel row)
    {
        using var doc = JsonDocument.Parse(row.ConfigJson!);
        return doc.RootElement.TryGetProperty(WebhookNotificationProvider.PayloadJsonKey, out var p) ? p.GetString() : null;
    }
    ...
    public async Task Webhook_NonSecretUrl_RoundTrips_Unmasked()
    {
        ...
        await LinkedTargetsAdminApi.PersistSchemaChannelsAsync(repo, profileId,
            Body(NotificationChannelTypes.Webhook, enabled: true,
                ("url", "https://hook.example/fire"),
                (WebhookNotificationProvider.PayloadJsonKey, """{"service":"{check}"}""")), registry, default);

        var rows = (await repo.GetMany()).ToList();
        var stored = rows.Single(r => r.ProviderType == NotificationChannelTypes.Webhook);
        Assert.AreEqual("""{"service":"{check}"}""", StoredPayloadJson(stored), "payload JSON template is persisted");
        ...
        Assert.AreEqual("""{"service":"{check}"}""", webhook.Config[WebhookNotificationProvider.PayloadJsonKey], "non-secret payload JSON is echoed on read");
    }
    ...
    [TestMethod]
    public async Task EnabledWebhook_InvalidPayloadJson_IsRejected()
    {
        using var conn = new SqliteConnection("Filename=:memory:");
        using var db = NewDb(conn);
        var repo = new Repository<AlertProfileChannel>(db);
        ...
        Assert.IsNotNull(reason, "an enabled webhook channel with malformed payload JSON is rejected");
        StringAssert.Contains(reason!, "Payload JSON");
    }
