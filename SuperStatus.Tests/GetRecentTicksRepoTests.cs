...
public class GetRecentTicksRepoTests
{
    ...
    public async Task EmptyHistory_ReturnsEmptyList()
    {
        ...
        using var _ = db;
        using var __ = conn;
        db.StatusCheckSet.Add(new StatusCheck { Id = 7, Title = "x", StatusCheckUrl = "x", WebHookOnErrorUrl = "x", ServiceLogoUrl = "x" });
        await db.SaveChangesAsync();
    ...
    public async Task CountClampedToSafeMaximum()
    {
        ...
        using var _ = db;
        using var __ = conn;
        db.StatusCheckSet.Add(new StatusCheck { Id = 1, Title = "x", StatusCheckUrl = "x", WebHookOnErrorUrl = "x", ServiceLogoUrl = "x" });
        for (int i = 0; i < 600; i++)
        {
            db.HistoricalStatusDataSet.Add(new HistoricalStatusData
            {
