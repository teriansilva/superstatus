...
public class LinkedTargetsPhaseATests
{
    ...
    private static AlertEvaluator Evaluator(SuperStatusDb db, RecordingEmailNotifier? email = null)
        => new(new StatusCheckLinkRepository(db),
            ...
            {
                new EmailNotificationProvider(email ?? new RecordingEmailNotifier()),
                new WebPushNotificationProvider(new NoopWebPushNotifier()),
            }),
    ...
    // #343 Phase 4: the webhook-dispatch tests (two-target throttle, disabled-target
    // skip, throttle-window skip) were removed — webhook delivery is folded into the
    // notification-channel model and fires through AlertEvaluator as a `webhook` channel
    // now (covered by the Phase-4 webhook-channel tests). RunPostStatusCheckTasks is an
    // inert seam; the no-links no-op above still holds.
