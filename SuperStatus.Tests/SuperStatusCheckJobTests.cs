...
public class SuperStatusCheckJobTests
{
    ...
    private sealed class FakeStatusCheckService : IStatusCheckService
    {
        ...
        public Task<CheckMetricsViewModel?> GetRecentMetricsAsync(long statusCheckId, int count, CancellationToken cancellationToken = default) => throw new NotImplementedException();
        public Task<bool> RecordHeartbeatAsync(string token, CancellationToken cancellationToken = default) => throw new NotImplementedException();
        public Task<string?> GetHeartbeatTokenAsync(long statusCheckId, CancellationToken cancellationToken = default) => throw new NotImplementedException();
        public Task<string?> RegenerateHeartbeatTokenAsync(long statusCheckId, CancellationToken cancellationToken = default) => throw new NotImplementedException();
        ...
        // #312: the job consults the resolve-or-disable gate before probing. The fake
        // always resolves to a runnable provider so the existing fan-out / #76 / #78
        // assertions still execute every check.
        public ProbeResolution ResolveProbe(StatusCheck statusCheck) => ProbeResolution.Ok(StubProvider.Instance, "{}");

        private sealed class StubProvider : ICheckProvider
        {
            public static readonly StubProvider Instance = new();
            public ProviderDescriptor Descriptor { get; } = new("stub", "Stub", "", new ConfigSchema(1, System.Array.Empty<ConfigField>()));
            public Task<ProbeResult> ProbeAsync(ProbeContext context, CancellationToken cancellationToken = default) => Task.FromResult(new ProbeResult());
        }
    }
}
