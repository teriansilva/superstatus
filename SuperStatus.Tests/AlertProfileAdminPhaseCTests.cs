...
public class AlertProfileAdminPhaseCTests
{
    ...
    public void ListPanel_RendersRows_WithUsedByCounts_PushMark_AndSiteDefaultFlag()
    {
        ...
        // The site-default profile renders "(site default)" + the flag tag, not a recipients list.
        Assert.IsTrue(cut.Markup.Contains("(site default)"));
        var flag = cut.Find(".ap-flag");
        Assert.AreEqual("SITE DEFAULT", flag.TextContent.Trim());
    ...
    public void EditDialog_EmailOn_NoRecipients_NoSiteDefault_BlockedClientSide_ThenSiteDefaultUnblocks()
    {
        ...
        // Email on + no recipients + no site default → blocked with a visible
        // message (mirrors the API's 422 rule), nothing posted.
        provider.FindAll(".mud-dialog-actions button").Single(b => b.TextContent.Trim() == "Save").Click();
        provider.WaitForAssertion(() => Assert.IsTrue(provider.Markup.Contains(
            "Email is enabled but the profile has no recipients — add recipients or turn on site default recipients.")));
        Assert.IsFalse(requests.Any(r => r.Method == "POST"), "the invalid combination never posts");

        // Turning the site-default toggle on resolves it — Save now posts and closes.
    ...
    public void EditDialog_SiteDefaultToggle_DisablesAndClearsRecipients()
    {
        ...
        // Mutually exclusive UX: site default on → recipients disabled AND cleared.
        provider.Find(".ap-sitedefault input").Change(true);

        provider.WaitForAssertion(() =>
        {
            var input = provider.Find(".ap-recipients input");
            Assert.IsTrue(input.HasAttribute("disabled"), "recipients field disabled while site default is on");
    ...
    public async Task AlertLog_ResolvesProfileName_ViaTheRepositoryJoin_NoSchemaChange()
    {
        ...
        db.AlertDeliveryLogSet.AddRange(
            new AlertDeliveryLog { StatusCheckId = check.Id, AlertProfileId = profile.Id, AttemptedUtc = DateTime.UtcNow, Channel = AlertChannel.Email, Trigger = AlertTrigger.Outage, Outcome = AlertOutcome.Fired },
            // pre-#291 row: no AlertProfileId → name resolves to null, UI shows "—".
            new AlertDeliveryLog { StatusCheckId = check.Id, AttemptedUtc = DateTime.UtcNow.AddMinutes(-1), Channel = AlertChannel.Email, Trigger = AlertTrigger.Outage, Outcome = AlertOutcome.Fired });
        await db.SaveChangesAsync();
    ...
    [TestMethod]
    public void AlertsPane_ProfilesPanelRendersFirst_AboveDeliveryConfigAndLog()
    {
        ...
        int email = alertsPane.IndexOf("Email alerts (SMTP)", StringComparison.Ordinal);
        ...
        Assert.IsTrue(email > profiles, "profiles panel sits above the email delivery config");
        Assert.IsTrue(log > email, "alert log stays at the bottom of the pane");
    }
