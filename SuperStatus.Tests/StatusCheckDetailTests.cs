...
/// #293 Phase D carve-out: the read-only SLA summary line shows the linked
/// SLA's slow threshold (approved mock) — display of the SLA contract, not
/// editable check config.
/// </summary>
[TestClass]
public class StatusCheckDetailTests
{
    ...
    private sealed class DetailStub : HttpMessageHandler
    {
        ...
        public DetailStub(string recentJson = "[]") => _recentJson = recentJson;
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                body = _recentJson; // #418: injectable recent ticks (default: empty → check found)
            }
    ...
    private static BunitTestContext Ctx(bool authenticated, string recentJson = "[]")
    {
        var ctx = new BunitTestContext();
        var auth = ctx.AddTestAuthorization();
        if (authenticated) auth.SetAuthorized("operator");
        ctx.Services.AddSingleton(new StatusApiClient(new HttpClient(new DetailStub(recentJson)) { BaseAddress = new Uri("http://api.test") }));
    ...
    private static void AssertNoConfig(BunitTestContext ctx)
    {
        ...
        // #293 Phase D: the effective slow threshold now DOES render — but
        // exactly once, inside the read-only SLA summary line (approved mock),
        // never as editable check config.
        StringAssert.Contains(cut.Find(".sla-card .sla-line").TextContent, "1234 ms");
        Assert.AreEqual(
            cut.Markup.IndexOf("1234 ms", StringComparison.Ordinal),
            cut.Markup.LastIndexOf("1234 ms", StringComparison.Ordinal),
            "the slow threshold appears only in the SLA summary line");
    ...
    // #418: a passing non-HTTP (AI/heartbeat) tick stores HttpStatusCode 0. The
    // tick-history dot must render UP — not down because 0 != ExpectedStatusCode.
    [TestMethod]
    public void TickHistory_HealthyNonHttpTick_RendersUp_NotDown()
    {
        var tick = new HistoricalStatusData
        {
            StatusCheckId = CheckId, TimeOfCheckUTC = DateTime.UtcNow.AddMinutes(-1),
            HttpStatusCode = 0, ResponseTimeInMs = 900, CheckFailed = false, FailType = FailType.NoFail,
        };
        var recentJson = JsonSerializer.Serialize(new[] { tick }, new JsonSerializerOptions(JsonSerializerDefaults.Web));
        using var ctx = Ctx(authenticated: false, recentJson: recentJson);

        var cut = ctx.RenderComponent<StatusCheckDetail>(p => p.Add(x => x.Id, CheckId));

        cut.WaitForAssertion(() =>
        {
            var cell = cut.FindAll("span.v").Single(e => e.TextContent.Trim() == "0 • 900 ms");
            Assert.IsTrue(cell.ClassList.Contains("up"),
                $"a healthy non-HTTP tick (HttpStatusCode 0, FailType NoFail) must render 'up'; got: {string.Join(" ", cell.ClassList)}");
            Assert.IsFalse(cell.ClassList.Contains("down"), "must not render as down");
        });
    }
