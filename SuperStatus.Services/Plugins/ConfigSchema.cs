            ...
            foreach (var field in Fields)
            {
                ...
                switch (field.Kind)
                {
                    ...
                    case ConfigFieldKind.Json:
                        if (value.ValueKind != JsonValueKind.String)
                            return $"'{field.Label}' must be JSON text";
                        var jsonText = value.GetString();
                    ...
                    case ConfigFieldKind.Number:
                        ...
                        // post numbers as strings). Decimals are valid — some Number fields
                        // are read as double (e.g. min tokens/sec); parse with
                        // InvariantCulture to match the client validator and stay
                        // locale-independent (#388).
                        if (value.ValueKind == JsonValueKind.Number) break;
                        if (value.ValueKind == JsonValueKind.String
                            && double.TryParse(value.GetString(), NumberStyles.Float,
                                CultureInfo.InvariantCulture, out _)) break;
