...
public sealed class DemoApiHandler : HttpMessageHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        // #433: the dry-run probe and model discovery reach out to an operator-supplied host.
        // In demo mode that must NEVER happen — the public demo would become an open outbound
        // relay — so they are answered from canned data here, before the table below.
        if (path == "/statuscheck/providers/ai/models")
            return Respond(request, JsonResponse(DemoData.AiModels()));
        if (path.StartsWith("/statuscheck/providers/", StringComparison.Ordinal) && path.EndsWith("/test", StringComparison.Ordinal))
            return Respond(request, JsonResponse(DemoData.CheckTestResult(ProviderTypeFromTestPath(path))));
    ...
    /// <summary>`/statuscheck/providers/{type}/test` → `{type}`.</summary>
    private static string ProviderTypeFromTestPath(string path)
    {
        var segments = path.Split('/', StringSplitOptions.RemoveEmptyEntries);
        // statuscheck / providers / {type} / test
        return segments.Length >= 4 ? Uri.UnescapeDataString(segments[2]) : string.Empty;
    }
