...
static void ConfigureEndpoints(WebApplication app)
{
    ...
    // #320 Phase 2b: the agent-heartbeat ping sink. ANONYMOUS by design — the token IS
    // the credential, so there is no user identity. GET or POST both work (curl/wget/cron
    ...
    // #320 Phase 2b: operator-only read of a heartbeat check's token, so the edit dialog
    // can render the ping URL. AUTHENTICATED — the token never rides the anonymous read VM
    ...
    // #320 Phase 2b: rotate the token (operator action). The old ping URL stops working the
