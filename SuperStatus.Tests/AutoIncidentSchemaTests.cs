...
public class AutoIncidentSchemaTests
{
    ...
    private static SqliteConnection OpenDb()
    {
        ...
        // #415: the FK on Incident.SourceStatusCheckId needs the referenced checks to
        // exist. Seed checks 1..5 so the fabricated source ids in these index tests
        // resolve (a fresh in-memory DB assigns sequential ids from 1).
        for (int i = 1; i <= 5; i++)
            db.StatusCheckSet.Add(new StatusCheck { Title = $"c{i}", StatusCheckUrl = "https://x/health", ServiceLogoUrl = "", Enabled = true, Created = DateTime.UtcNow });
        db.SaveChanges();
        return conn;
    }
    ...
    public void AutoIncidentEnabled_RoundTripsThroughViewModel()
    {
        ...
        {
            Title = "probe", StatusCheckUrl = "https://x/health",
