            ...
            await Parallel.ForEachAsync(
                ...
                async (checkId, token) =>
                {
                    ...
                    try
                    {
                        ...
                        // #312: the single resolve-or-disable gate (shared with run-now).
                        // An unknown provider type or invalid config disables the check
                        // calmly — skip the probe entirely (no tick recorded, no default
                        // probe), surfaced to the console via the view model. Never a crash.
                        var resolution = statusCheckService.ResolveProbe(statusCheck);
                        if (resolution.IsDisabled)
                        {
                            logger.LogInformation("Skipping check {Title} ({Id}) — disabled: {Reason}", statusCheck.Title, statusCheck.Id, resolution.DisableReason);
                            return;
                        }
                        ...
                        // Issue #241/#253: evaluate per-check alert rules immediately
                        // after the failure-state update (alongside the auto-incident
                        // step), BEFORE the webhook post-tasks — so a slow/throwing
                        // webhook can't skip the alert audit/decision for this tick.
                        // Runs every tick (incl. recovery on a healthy result); records
                        // the decision to AlertDeliveryLog. Delivery is a no-op in Phase A.
                        var alertEvaluator = scope.ServiceProvider.GetRequiredService<SuperStatus.Services.Alerts.IAlertEvaluator>();
                        await alertEvaluator.EvaluateAsync(statusCheck, statusCheckResult.FailType, token);
