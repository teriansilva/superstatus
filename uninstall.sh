...
if [ "$HAVE_COMPOSE" = yes ] && [ "${SUPERSTATUS_KEEP_DIR:-}" != 1 ]; then
  ...
  # docker-compose.watchtower.yml is always written by install.sh (so the overlay
  # can be enabled later without re-downloading), so it must be removed too —
  # otherwise it orphans the directory and the rmdir below leaves it behind.
  rm -f "$DIR/docker-compose.yml" "$DIR/docker-compose.watchtower.yml" "$DIR/.env" "$DIR/postgres/init-databases.sh" 2>/dev/null || true
