namespace SuperStatus.Data.Entities
{
    ...
    public class SiteSettings : EntityBase
    {
        ...
        // ---- Issue #334: self-serve automatic updates ----
        //
        // The cadence lives here, not in Watchtower: the updater ships as a pure
        // on-demand executor, so switching this off in the console really stops
        // updates. Editable from the admin Updates panel; effective without a redeploy.

        /// <summary>Whether the daily automatic update runs. Off by default — an
        /// unattended pull+restart is opt-in, even though the engine ships by default.</summary>
        public bool AutoUpdateEnabled { get; set; }

        /// <summary>Time of day the automatic update fires, in UTC. v1 is UTC-only and
        /// labelled as such in the console (local-time entry is a follow-up).</summary>
        public TimeOnly AutoUpdateTimeUtc { get; set; } = new(3, 0);

        /// <summary>When the automatic update last fired and was ACCEPTED by the updater.
        /// De-dupes the daily run. Deliberately not stamped on a failed trigger, so a
        /// transient token/network error retries later the same day rather than
        /// suppressing updates until tomorrow. Null = never run.</summary>
        ...
        // ---- Issue #358: self-host dynamic-issuer hardening ----

        /// <summary>The operator's allowed sign-in hosts as a JSON array of strings
        /// (each a <c>host</c> or <c>host:port</c>). Drives the self-host dynamic
        /// OIDC issuer from relaxed (empty ⇒ accept any request host, first-run) to
        /// hardened (non-empty ⇒ only these hosts are honored as the login issuer).
        /// Ignored entirely in the env-pinned (cloud/reverse-proxy) mode. Not secret
        /// — these are the public URLs users already type. Normalized/validated by
        /// the AuthHostAllowlist helper (SuperStatus.ServiceDefaults) on save.</summary>
        public string AllowedAuthHostsJson { get; set; } = "[]";
    }
}
