...
namespace SuperStatus.Services
{
    ...
    public static class StatusCheckHttpClientRegistration
    {
        public static IServiceCollection AddStatusCheckHttpClients(this IServiceCollection services)
        {
            ...
            // #317: AI/LLM canary check client. No fixed client timeout — the engine
            // bounds each probe via the provider's ProbeTimeout. Same pooled/instrumented
            // handler as the others.
            services.AddHttpClient(StatusCheckHttpClients.AiCheck, c => c.Timeout = Timeout.InfiniteTimeSpan)
                    .ConfigurePrimaryHttpMessageHandler(CreateHandler);

            // #433: AI model-discovery client. Unlike the canary this one keeps a real client
            // timeout — it is an interactive call with an operator waiting, and its own linked
            // CancellationTokenSource is the primary bound; the client timeout is the backstop.
            services.AddHttpClient(StatusCheckHttpClients.AiModels, ConfigureClient)
                    .ConfigurePrimaryHttpMessageHandler(CreateHandler);
