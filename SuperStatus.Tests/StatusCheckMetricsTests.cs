...
public class StatusCheckMetricsTests
{
    ...
    [TestMethod]
    public async Task RunPostStatusCheckTasks_WebhookFires_EmitsWebhooksFired_WithSuccessOutcome()
    {
        var (db, conn) = Relational();
        using var _ = db; using var __ = conn;
        ...
        var failed = new HistoricalStatusData(new StatusCheckResult(check, 0, 0, true), FailType.Unreachable);
        await Service(db, _ => new HttpResponseMessage(HttpStatusCode.OK)).RunPostStatusCheckTasks(check, failed);
        listener.Dispose();

        var fired = sink.Single(m => m.Name == "statuschecks.webhooks_fired");
        Assert.AreEqual(1d, fired.Value);
        Assert.AreEqual(nameof(WebhookOutcome.Success), fired.Tags["outcome"]);
    }
