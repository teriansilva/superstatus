...
namespace SuperStatus.Services.Services
{
    ...
    public class StatusCheckService(IStatusCheckRepository statusCheckRepository, IHistoricalStatusDataRepository historicalStatusDataRepository, IHistoricalStatusActionRepository historicalStatusActionRepository, IWebhookExecutionLogRepository webhookExecutionLogRepository, IDailyStatusRollupRepository dailyStatusRollupRepository, IHttpClientFactory httpClientFactory, ILogger<StatusCheckService> logger, IAutoIncidentCoordinator? autoIncidentCoordinator = null, IStatusCheckLinkRepository? statusCheckLinkRepository = null, ICheckProviderRegistry? checkProviderRegistry = null) : IStatusCheckService
    {
        ...
        public async Task<HistoricalStatusData> ExecuteStatusCheck(StatusCheck statusCheck)
        {
            ...
            {
                // Null in Phase 1 (no provider emits metrics yet).
        ...
        // #312: hard backstop beyond the named client's own timeout. A provider that
        // ignores cancellation is abandoned after this (via WaitAsync) so it can never
        // pin the scheduler/run-now scope. Settable so a regression can drive it short.
        public TimeSpan ProbeBackstop { get; set; } = TimeSpan.FromSeconds(StatusCheckHttpClients.TimeoutSeconds + 5);

        // #312: hard probe containment. A per-probe timeout (the named client's 10s
        // ceiling) plus a WaitAsync backstop and a try/catch convert a provider that
        // throws, hangs, OR ignores cancellation into a normalized Unreachable result —
        // it can never propagate into the scheduler tick, backoff, auto-incident, or alerts.
        private async Task<ProbeResult> RunProbeSafelyAsync(ICheckProvider provider, StatusCheck check, string configJson)
        {
            // The provider sees the named client's own ceiling (the HTTP client's 10s
            // still fires first on a normal hang, preserving the exact timeout shape).
            var context = new ProbeContext(check.Id, check.Title, configJson, TimeSpan.FromSeconds(StatusCheckHttpClients.TimeoutSeconds));

            using var cts = new CancellationTokenSource(ProbeBackstop);
            try
            {
