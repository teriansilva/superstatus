...
public class StatusApiClient(HttpClient httpClient)
{
    ...
    /// <summary>#433: dry-run one probe against the config currently typed in the edit dialog.
    /// Nothing is saved. Returns null when the call fails outright so the dialog can say
    /// "couldn't run the test" rather than crash the circuit; a probe that ran and found the
    /// target down is a successful call with a <c>down</c> outcome, not a null.</summary>
    public async Task<CheckTestResultViewModel?> TestCheckProviderAsync(
        string providerType, long checkId, IReadOnlyDictionary<string, string> config, CancellationToken cancellationToken = default)
    {
        var payload = new CheckTestRequest
        {
            CheckId = checkId,
            Config = config.ToDictionary(kv => kv.Key, kv => kv.Value),
        };
        try
        {
            using var response = await httpClient.PostAsJsonAsync(
                $"/statuscheck/providers/{Uri.EscapeDataString(providerType)}/test", payload, cancellationToken);

            if (!response.IsSuccessStatusCode)
            {
                // 422 carries the schema reason ("missing required 'Model'"), which is exactly what
                // the operator needs to see — surface it as a down verdict rather than swallowing it.
                var reason = await ReadProblemMessageAsync(response, cancellationToken);
                return reason is null ? null : new CheckTestResultViewModel { Outcome = "down", Ok = false, Message = reason };
            }
            return await response.Content.ReadFromJsonAsync<CheckTestResultViewModel>(cancellationToken: cancellationToken);
        }
        catch (Exception ex) when (IsCallFailure(ex, cancellationToken))
        {
            // The dialog's handlers have only `finally` blocks, so anything thrown here would
            // escape a Blazor event and fault the circuit — taking the whole dialog down instead
            // of showing "the test could not be run". A refused connection, a timeout, a body
            // that disconnects mid-read, or malformed 2xx JSON must all land as a null.
            return null;
        }
    }

    /// <summary>#433: ask an OpenAI-compatible endpoint which models it serves, so the dialog can
    /// offer a list instead of a free-text guess. A blank key reuses the check's stored one.</summary>
    public async Task<AiModelsResultViewModel?> DiscoverAiModelsAsync(
        long checkId, string baseUrl, string? apiKey, CancellationToken cancellationToken = default)
    {
        var payload = new AiModelsRequest { CheckId = checkId, BaseUrl = baseUrl, ApiKey = apiKey };
        try
        {
            using var response = await httpClient.PostAsJsonAsync("/statuscheck/providers/ai/models", payload, cancellationToken);
            if (!response.IsSuccessStatusCode)
            {
                var reason = await ReadProblemMessageAsync(response, cancellationToken);
                return new AiModelsResultViewModel { Ok = false, SupportsListing = false, Message = reason ?? "Could not reach the endpoint." };
            }
            return await response.Content.ReadFromJsonAsync<AiModelsResultViewModel>(cancellationToken: cancellationToken);
        }
        catch (Exception ex) when (IsCallFailure(ex, cancellationToken))
        {
            return new AiModelsResultViewModel { Ok = false, SupportsListing = false, Message = "Could not reach the endpoint." };
        }
    }

    /// <summary>#433: the failure modes these two calls must absorb rather than propagate —
    /// transport refusal, timeout, a body that disconnects mid-read, and malformed 2xx JSON.
    /// A genuine caller cancellation is deliberately NOT absorbed: that is the dialog going
    /// away, not the call failing, and swallowing it would install a bogus verdict.</summary>
    private static bool IsCallFailure(Exception ex, CancellationToken cancellationToken)
    {
        if (ex is OperationCanceledException && cancellationToken.IsCancellationRequested) return false;
        return ex is HttpRequestException
            or TaskCanceledException            // HttpClient timeout surfaces as this
            or OperationCanceledException       // …and as this on some paths
            or System.Text.Json.JsonException
            or System.IO.IOException;
    }

    /// <summary>Pull the <c>message</c> out of an API error body ({ "message": "…" }), or null
    /// when the body isn't that shape.</summary>
    private static async Task<string?> ReadProblemMessageAsync(HttpResponseMessage response, CancellationToken cancellationToken)
    {
        ...
        {
            // #433: a blanket catch here silently defeated the lifecycle contract above. The
            // error status arrives, then the dialog closes (or an edit supersedes the call)
            // while its BODY is still streaming — cancellation must travel up, not be turned
            // into a neutral "no message" that lets the caller report a verdict for a request
            // nobody is waiting on.
            throw;
        }
        catch
        {
            // Any other malformed/absent body is genuinely "no message to show".
            return null;
        }
    }

    /// <summary>#343 Phase 2: the registered notification channels (delivery sibling of
    /// the check providers), for the Plugins page's "Notification channels" catalogue.
    /// Returns an empty list on failure so the page degrades to a calm empty state.</summary>
    public async Task<List<NotificationDescriptorViewModel>> GetNotificationProvidersAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            return await httpClient.GetFromJsonAsync<List<NotificationDescriptorViewModel>>("/notifications/providers", cancellationToken)
                ?? new List<NotificationDescriptorViewModel>();
        }
        catch
        {
            return new List<NotificationDescriptorViewModel>();
        }
    }

    /// <summary>#365: fire a one-off test send through a notification channel. Posts the
    /// operator's typed config + the owning profile id; the API builds the effective config
    /// (a blank secret reuses the stored credential), validates it, and actually delivers.
    /// Returns <c>(ok, message)</c> for the Snackbar — 404 ⇒ not registered, 422 ⇒ the
    /// validation / unsupported message, else the mapped send outcome. Never throws.</summary>
    public async Task<(bool Ok, string Message)> TestNotificationChannelAsync(
        string providerType, long profileId, IReadOnlyDictionary<string, string> config, CancellationToken cancellationToken = default)
    {
        try
        {
            var payload = new ChannelTestRequest { ProfileId = profileId, Config = new Dictionary<string, string>(config) };
            using var response = await httpClient.PostAsJsonAsync(
                $"/notifications/providers/{Uri.EscapeDataString(providerType)}/test", payload, cancellationToken);

            if (!response.IsSuccessStatusCode)
            {
                var problem = await response.Content.ReadFromJsonAsync<DeleteConflictBody>(cancellationToken: cancellationToken);
                return (false, problem?.Message ?? $"Test failed ({(int)response.StatusCode}).");
            }

            var result = await response.Content.ReadFromJsonAsync<ChannelTestResultViewModel>(cancellationToken: cancellationToken);
            if (result is null) return (false, "The test returned no result.");
            return result.Outcome switch
            {
                "sent" => (true, "Test alert sent — check the channel."),
                "skipped" => (false, $"Nothing sent: {result.Detail ?? "the channel had nothing to deliver"}."),
                _ => (false, $"Test failed: {result.Detail ?? "the channel could not deliver"}."),
            };
        }
        catch (Exception ex)
        {
    ...
    /// <summary>#342: batch add — create many checks from a pasted target list. Returns the
    /// per-line breakdown on success; throws with the server's message on a 422 rejection
    /// (unknown provider / not batch-capable, over-cap, unknown id, no valid targets) so the
    /// dialog can surface it verbatim.</summary>
    public async Task<BatchCreateChecksResponse> BatchCreateChecksAsync(BatchCreateChecksRequest request, CancellationToken cancellationToken = default)
    {
        using var response = await httpClient.PostAsJsonAsync("/statuscheck/batch", request, cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            var content = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new HttpRequestException(TryExtractMessage(content) ?? $"{(int)response.StatusCode} {response.ReasonPhrase}");
        }
        return await response.Content.ReadFromJsonAsync<BatchCreateChecksResponse>(cancellationToken: cancellationToken)
            ?? new BatchCreateChecksResponse();
    }

    // Pull the { "message": "…" } out of a 422 body so the dialog shows the reason, not raw JSON.
    private static string? TryExtractMessage(string body)
    {
        ...
        catch { /* non-JSON body — fall back to the status line */ }
        return null;
    }
