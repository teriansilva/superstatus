...
namespace SuperStatus.Services
{
    ...
    public static class ServiceRegistration
    {
        public static void AddApplicationServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            // #433: AI model discovery for the edit dialog's guided setup. Stateless, depends
            // only on the singleton IHttpClientFactory — same shape as the providers above.
            services.AddSingleton<SuperStatus.Services.Providers.Ai.IAiModelDiscovery, SuperStatus.Services.Providers.Ai.AiModelDiscovery>();
            ...
            // Issue #311/#334: in-app "Update now" → Watchtower's authenticated http-api
            // (/v1/update). The app never touches the Docker socket; it just POSTs the
            // trigger. The URL + token come from the api env, layered on by the update
            // engine (docker-compose.watchtower.yml) which the DEFAULT install includes;
            // absent ⇒ the operator opted out ⇒ CanApply is false ⇒ the console offers the
            ...
            // #430: Matrix (client-server API, PUT + bearer token). Same named HttpClient again.
            services.AddScoped<SuperStatus.Services.Notifications.INotificationProvider, SuperStatus.Services.Notifications.MatrixNotificationProvider>();
