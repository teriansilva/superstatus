...
public class SettingsApiClient(HttpClient httpClient)
{
    ...
    /// <summary>#358: persist ONLY the allowed-sign-in-hosts allowlist (the dedicated
    /// /settings/authhosts endpoint, so it can't wipe branding/AI/SMTP and they can't
    /// wipe it). Returns the server-normalized result. Throws on non-success.</summary>
    public async Task<SiteSettingsViewModel> SaveAuthHostsSettingsAsync(SiteSettingsViewModel settings, CancellationToken cancellationToken = default)
    {
        using var response = await httpClient.PostAsJsonAsync("/settings/authhosts", settings, cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            var content = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new HttpRequestException($"Failed to save allowed sign-in hosts. {(int)response.StatusCode} {response.ReasonPhrase}. {content}");
        }
        return await response.Content.ReadFromJsonAsync<SiteSettingsViewModel>(cancellationToken: cancellationToken)
               ?? settings;
    }
