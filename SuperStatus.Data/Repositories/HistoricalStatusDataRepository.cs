...
namespace SuperStatus.Data.Repositories
{
    public interface IHistoricalStatusDataRepository : IRepository<HistoricalStatusData>
    {
        ...
        /// </summary>
        Task<List<DailyStateRollup>> GetDailyStateRollupAsync(long statusCheckId, long expectedResponseTimeMs, DateTime sinceUtc, CancellationToken cancellationToken = default);
    ...
    public class HistoricalStatusDataRepository : Repository<HistoricalStatusData>, IHistoricalStatusDataRepository
    {
        ...
        public async Task<List<DailyStateRollup>> GetDailyStateRollupAsync(long statusCheckId, long expectedResponseTimeMs, DateTime sinceUtc, CancellationToken cancellationToken = default)
        {
            ...
            //
            // #418: classify from the provider-computed FailType, NOT from the HTTP
            // status code. Every provider (HTTP, AI, heartbeat, …) stores a FailType;
            // only HTTP sets a real HttpStatusCode, so an "HttpStatusCode == expected"
            // rule marked every healthy non-HTTP tick down. Slowness is still
            // re-evaluated against the current slow threshold (latency is universal),
            // so a changed SLA re-colours history exactly as before.
            return await DbSet
                ...
                .Select(g => new DailyStateRollup(
                    g.Key,
                    g.Count(),
                    g.Count(x => x.FailType == FailType.StatusCode || x.FailType == FailType.Unreachable),
                    g.Count(x => x.FailType != FailType.StatusCode && x.FailType != FailType.Unreachable && x.ResponseTimeInMs > expectedResponseTimeMs),
                    g.Count(x => x.FailType == FailType.Unreachable)))
        ...
        public async Task<List<CheckDailyStateRollup>> GetRecentDailyStateForChecksAsync(IReadOnlyCollection<long> statusCheckIds, DateTime sinceUtc, CancellationToken cancellationToken = default)
        {
            ...
            // each check's own thresholds, so the result is identical to the
            // per-check GetDailyStateRollupAsync — just batched. EF emits
            // COUNT(*) FILTER (Npgsql) / COUNT(CASE …) (SQLite), plans on the
            // (StatusCheckId, TimeOfCheckUTC) composite. #293: the slow
            // threshold comes from the check's linked SLA (the Sla navigation
            // adds one more join); after the startup backfill every check has
            // one, so the LEFT JOIN never misses in practice.
            return await (
                from h in DbSet
                ...
                select new CheckDailyStateRollup(
                    ...
                    g.Key.Day,
                    g.Count(),
                    // #418: classify from FailType, not the HTTP status code (see the
                    // per-check overload). The join to StatusCheckSet remains only for
                    // the per-check slow threshold (Sla.SlowThresholdMs), which stays
                    // re-evaluated so a changed SLA re-colours history.
                    g.Count(x => x.h.FailType == FailType.StatusCode || x.h.FailType == FailType.Unreachable),
                    g.Count(x => x.h.FailType != FailType.StatusCode && x.h.FailType != FailType.Unreachable && x.h.ResponseTimeInMs > x.c.Sla!.SlowThresholdMs),
                    g.Count(x => x.h.FailType == FailType.Unreachable)))
                .ToListAsync(cancellationToken);
        }
