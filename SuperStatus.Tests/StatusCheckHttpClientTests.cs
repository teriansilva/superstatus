...
public class StatusCheckHttpClientTests
{
    ...
    // ---- RunPostStatusCheckTasks uses the webhook client + audit ----------

    [TestMethod]
    public async Task Webhook_Success_UsesWebhookClient_LogsSuccess_ReturnsAction()
    {
        var (db, conn) = Relational();
        using var _ = db; using var __ = conn;
        var check = PersistCheck(db, expectedStatus: 200, webhook: true);
        var factory = new RecordingHttpClientFactory(_ => new HttpResponseMessage(HttpStatusCode.OK));
        var svc = Service(db, factory);

        var action = await svc.RunPostStatusCheckTasks(check, FailedData(check));

        CollectionAssert.Contains(factory.RequestedNames, StatusCheckHttpClients.Webhook);
        Assert.IsNotNull(action);
        Assert.AreEqual(WebhookOutcome.Success, await LatestOutcome(db));
    }

    [TestMethod]
    public async Task Webhook_NonSuccess_LogsNonSuccess_ReturnsNull()
    {
        var (db, conn) = Relational();
        using var _ = db; using var __ = conn;
        var check = PersistCheck(db, expectedStatus: 200, webhook: true);
        var factory = new RecordingHttpClientFactory(_ => new HttpResponseMessage(HttpStatusCode.InternalServerError));
        var svc = Service(db, factory);

        var action = await svc.RunPostStatusCheckTasks(check, FailedData(check));

        Assert.IsNull(action);
        Assert.AreEqual(WebhookOutcome.NonSuccess, await LatestOutcome(db));
    }

    [TestMethod]
    public async Task Webhook_Timeout_LogsTimeout_ReturnsNull()
    {
        var (db, conn) = Relational();
        using var _ = db; using var __ = conn;
        var check = PersistCheck(db, expectedStatus: 200, webhook: true);
        ...
        Assert.IsNull(action);
        Assert.AreEqual(WebhookOutcome.Timeout, await LatestOutcome(db));
    }
    ...
    private static StatusCheck PersistCheck(SuperStatusDb db, int expectedStatus, bool webhook = false)
    {
        var check = new StatusCheck
        {
            ...
            ExpectedResponseTimeInMs = 60_000,           // generous so timing never trips ResponseTime
            Enabled = true,
            IsWebHookOnErrorEnabled = webhook,
            WebHookOnErrorUrl = webhook ? "http://hook.test/fire" : string.Empty,
            ThrottleWebHookToExecuteOnlyEveryXMinutes = 0,
