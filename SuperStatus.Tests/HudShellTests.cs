...
public class HudShellTests
{
    ...
    [TestMethod]
    public void HeaderBar_RendersBrand_AndClock()
    {
        using var ctx = CreateContextWith(authenticated: false);
        // The seeded default title cascades a wordmark; #286 removed the
        // STATUS//UP gimmick tag, so the bar is brand + clock + actions.
        var cut = ctx.RenderComponent<HeaderBar>(p => p
            .AddCascadingValue("SiteSettings",
                new SuperStatus.Data.ViewModels.SiteSettingsViewModel { Title = "SuperStatus" }));

        cut.Find(".hud-topbar");
        cut.Find(".brand .name");
        // Clock cell is present (text format checked elsewhere).
        cut.Find(".clock");
        // #286: the STATUS//UP gimmick tag is gone.
        Assert.AreEqual(0, cut.FindAll(".topbar-status").Count, "STATUS//UP tag was removed (#286).");
    }
