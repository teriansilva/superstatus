...
// Identity (discovery, token, userinfo, JWKS). IDP_PUBLIC_HTTP is the pinned
// browser-facing issuer when set; it falls back to IDP_HTTP when unset. When the
// two differ (a reverse proxy in front of the compose stack pins a public issuer),
// Identity advertises IDP_PUBLIC_HTTP and IdpBackchannelRewriteHandler retargets
// this service's own back-channel calls from the public host to the reachable
// internal one. #358/#363: on the no-proxy self-host stack both IDP_PUBLIC_HTTP and
// WEBAPP_HTTP are unset — Identity PINS the issuer to the internal host and Web only
// rebuilds the browser-facing redirect from the request host (idpDynamic below).
...
// #358/#363: dynamic self-host mode — the no-proxy two-port stack (IDP_PUBLIC_HTTP
// AND WEBAPP_HTTP both unset). Identity pins its issuer to the INTERNAL host (IDP_HTTP)
// so the authorize and token endpoints agree on it (else ID2088). The ONLY dynamic
// part is the browser-facing authorize/logout redirect, rebuilt from the request host
// on the published identity port; metadata / token / JWKS resolve over the internal
// authority (options.Authority = idpPublic, which falls back to idpInternal). NOT the
// reverse-proxy shape, which leaves IDP_PUBLIC_HTTP unset but sets WEBAPP_HTTP — see
// IdpAuthority.IsDynamicSelfHost.
var idpDynamic = IdpAuthority.IsDynamicSelfHost(
    Environment.GetEnvironmentVariable("IDP_PUBLIC_HTTP"),
    Environment.GetEnvironmentVariable("WEBAPP_HTTP"));
var identityPort =
    int.TryParse(Environment.GetEnvironmentVariable("IDENTITY_PORT"), out var parsedIdentityPort)
    && parsedIdentityPort is > 0 and <= 65535
        ? parsedIdentityPort
        : 8081;
...
.AddOpenIdConnect(OpenIdConnectDefaults.AuthenticationScheme, options =>
{
    ...
    // #358: dynamic self-host mode. options.Authority (above) is the INTERNAL
    // authority, and Identity pins its issuer to that same internal host, so
    // metadata / token / JWKS all resolve over the container network AND the
    // authorize-response iss + the id_token iss match the metadata issuer — the
    // framework's default issuer validation passes, and the code→token exchange
    // agrees on the issuer (no ID2088). The ONLY thing that must be dynamic is the
    // browser-facing authorize/logout redirect: rebuild it from the live request
    // host on the published identity port so the browser reaches a routable URL.
    // Nonce / PKCE / signature / audience are unchanged. None of this runs in pinned mode.
    if (idpDynamic)
    {
        ...
        options.Events.OnRedirectToIdentityProvider = context =>
        {
            ...
            context.ProtocolMessage.IssuerAddress =
                $"{request.Scheme}://{request.Host.Host}:{identityPort}/connect/authorize";
        ...
        options.Events.OnRedirectToIdentityProviderForSignOut = context =>
        {
            var request = context.HttpContext.Request;
            context.ProtocolMessage.IssuerAddress =
                $"{request.Scheme}://{request.Host.Host}:{identityPort}/connect/logout";
            return Task.CompletedTask;
        };
    }
});
...
// #358: expose the self-host dynamic-issuer mode to the console (drives the
// Access & security allowlist editor + the unpinned-issuer security banner).
builder.Services.AddSingleton<IssuerModeInfo>();

// #377: expose PUBLIC_DEMO to the layouts (drives DemoBanner + its reset countdown).
// Unrelated to SUPERSTATUS_DEMO below, which fakes the API data in Development.
builder.Services.AddSingleton<DemoModeInfo>();
