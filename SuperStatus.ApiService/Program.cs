...
if (DemoMode.IsEnabledFromEnvironment())
{
    ...
    var demoSettingsService = demoScope.ServiceProvider.GetRequiredService<ISiteSettingsService>();
    await demoSettingsService.CompleteOnboardingAsync();

    // Demo branding: give the public status page a title + tagline instead of an empty
    // header ("SuperStatus — Because Status Matters!"). Seed only when the title is unset
    // so an operator's mid-session edit stands until the next hourly reset wipes the DB.
    var demoSettings = await demoSettingsService.GetSettingsAsync();
    if (string.IsNullOrWhiteSpace(demoSettings.Title))
    {
        demoSettings.Title = "SuperStatus";
        demoSettings.Subtitle = "Because Status Matters!";
        await demoSettingsService.SaveSettingsAsync(demoSettings);
    }
}
...
static void ConfigureEndpoints(WebApplication app)
{
    ...
    // #433: operator-only dry-run probe + AI model discovery, so a check config can be tried
    // before it is saved. Nothing is persisted — no history, rollups, backoff or alerts.
    app.MapCheckProviderTestApi();
