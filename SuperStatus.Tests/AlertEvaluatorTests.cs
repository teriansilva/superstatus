...
public class AlertEvaluatorTests
{
    ...
    private sealed class StubEmailNotifier : IEmailNotifier
    {
        ...
        public Task<EmailSendResult> SendAlertAsync(StatusCheck check, AlertTrigger trigger, CancellationToken cancellationToken = default)
        {
            Calls++;
    ...
    public async Task WebPushFire_sends_recordsFiredWithDeviceTarget()
    {
        ...
        var check = AddCheck(db, c => { c.EmailAlertsEnabled = false; c.WebPushAlertsEnabled = true; });
    ...
    private static StatusCheck AddCheck(SuperStatusDb db, Action<StatusCheck>? configure = null)
    {
    ...
    public async Task Fires_once_perOutageEpisode_thenRecovers()
    {
        ...
        Assert.AreEqual(AlertOutcome.Fired, row.Outcome);
        Assert.IsNotNull(check.AlertedOutageDownSinceUtc);
        ...
        Assert.IsNull(check.AlertedOutageDownSinceUtc, "episode cleared");
    }
    ...
    public async Task ThrottledOutage_logsOneSkipped_neverRepeatsOrFiresLater()
    {
        ...
        Assert.IsNotNull(check.AlertedOutageDownSinceUtc, "episode consumed");
        ...
        check.AlertLastFiredUtc = DateTime.UtcNow.AddMinutes(-30);
        await db.SaveChangesAsync();
        await eval.EvaluateAsync(check, FailType.StatusCode);
        Assert.AreEqual(1, await RowCount(db));
    ...
    public async Task SuppressedThenHealthy_clearsMarker_withoutRecoveryLog()
    {
        ...
        await db.SaveChangesAsync();
        await eval.EvaluateAsync(check, FailType.NoFail);
        Assert.IsNull(check.AlertedOutageDownSinceUtc, "stale marker cleared on recovery");
    ...
    {
        var (eval, db, conn) = Build(); using var _ = db; using var __ = conn;
        var check = AddCheck(db, c => c.AlertOnFailureThreshold = 1);
        ...
        // Channel disabled, then the check recovers — the stale marker must still be
        // cleared so re-enabling a channel later can't replay an old recovery.
        check.EmailAlertsEnabled = false;
        check.ConsecutiveFailures = 0; check.DownSinceUtc = null;
        await db.SaveChangesAsync();
        await eval.EvaluateAsync(check, FailType.NoFail);
        Assert.IsNull(check.AlertedOutageDownSinceUtc, "stale marker cleared even with channels off");
    }
    ...
    {
        var (eval, db, conn) = Build(); using var _ = db; using var __ = conn;
        var check = AddCheck(db, c => { c.EmailAlertsEnabled = false; c.WebPushAlertsEnabled = false; });
    ...
    {
        var (eval, db, conn) = Build(); using var _ = db; using var __ = conn;
        var check = AddCheck(db, c => { c.EmailAlertsEnabled = true; c.WebPushAlertsEnabled = true; });
