...
/// driven by the [hidden] attribute). Verifies the Alerts tab keeps alert profiles,
/// and the alert log; #291 Phase B adds the Webhooks tab (between Incidents and
/// Alerts) which now owns the webhook log.
/// </summary>
[TestClass]
public class AdminTabsTests
{
    ...
    public void ClickingAlertsTab_showsAlertsPane_hidesOverview()
    {
        ...
        Assert.IsTrue(cut.Markup.Contains("ALERT PROFILES"), "alert profiles live under Alerts");
        Assert.IsTrue(cut.Markup.Contains("ALERT LOG"), "alert log lives under Alerts");
        Assert.IsFalse(cut.Markup.Contains("Provider settings moved"), "email provider pointer removed from Alerts");
        Assert.IsFalse(cut.Markup.Contains("Browser notifications moved"), "web-push provider pointer removed from Alerts");
    }
