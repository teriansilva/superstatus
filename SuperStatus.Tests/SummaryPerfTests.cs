...
public class SummaryPerfTests
{
    ...
    {
        var c = new StatusCheck { Title = "svc", StatusCheckUrl = "x", ServiceLogoUrl = "", ExpectedStatusCode = expectedStatus, Enabled = true, IntervalSeconds = 30, Created = DateTime.UtcNow };
        db.StatusCheckSet.Add(c); db.SaveChanges(); return c;
    }

    // #418: set FailType per tick (as production does) — the rollup now classifies
    // from it, not the HTTP status. Slowness stays latency-driven (re-evaluated).
    ...
    public async Task DailyRollup_CountsPerDay_DownDegradedTotal()
    {
        ...
        var rollup = await new HistoricalStatusDataRepository(db)
            .GetDailyStateRollupAsync(c.Id, 1000, d1.Date);
    ...
    // #418: a non-HTTP provider (AI/heartbeat) stores HttpStatusCode 0. The rollup
    // must classify from the provider-computed FailType — a healthy (NoFail) tick
    // counts UP, not down just because 0 != ExpectedStatusCode. Regression for the
    // "all-red strip / Uptime 0%" on a passing AI check.
    [TestMethod]
    public async Task DailyRollup_NonHttpCheck_HealthyTicksAreUp_HttpStatusCodeIgnored()
    {
        var (db, conn) = Relational(); using var _ = db; using var __ = conn;
        var c = Check(db, expectedStatus: 200, expectedRt: 3000);
        var d = new DateTime(2026, 5, 27, 8, 0, 0, DateTimeKind.Utc);
        // 4 healthy AI ticks (HttpStatusCode 0, FailType NoFail, ~900ms) + 1 genuine
        // down tick (bad content → FailType.StatusCode, still HttpStatusCode 0).
        for (int i = 0; i < 4; i++)
            db.HistoricalStatusDataSet.Add(new HistoricalStatusData { StatusCheckId = c.Id, TimeOfCheckUTC = d.AddMinutes(i), HttpStatusCode = 0, ResponseTimeInMs = 900, CheckFailed = false, FailType = FailType.NoFail });
        db.HistoricalStatusDataSet.Add(new HistoricalStatusData { StatusCheckId = c.Id, TimeOfCheckUTC = d.AddMinutes(4), HttpStatusCode = 0, ResponseTimeInMs = 900, CheckFailed = true, FailType = FailType.StatusCode });
        await db.SaveChangesAsync();

        var day = (await new HistoricalStatusDataRepository(db).GetDailyStateRollupAsync(c.Id, 3000, d.Date)).Single();

        Assert.AreEqual(5, day.Total);
        Assert.AreEqual(1, day.Down, "only the FailType.StatusCode tick is down — HttpStatusCode 0 must not down the healthy AI ticks");
        Assert.AreEqual(0, day.Degraded, "900ms < 3000ms threshold → not slow");
        Assert.AreEqual(4, day.Total - day.Down - day.Degraded, "the 4 healthy non-HTTP ticks count up");
    }
    ...
    [TestMethod]
    public async Task Summary_Uptime30dPct_IsTickLevel_NotGreenDayShare()
    {
        // Regression: Uptime30dPct counted fully-green DAYS, so a single blip
        // tick anywhere in a day zeroed that whole day's contribution — the
        // hero read 0.0% for a near-perfect young check. The headline must be
        // tick-level (ok / total), same semantic as the day tooltip's UptimePct.
        var (db, conn) = Relational(); using var _ = db; using var __ = conn;
        var c = Check(db);
        var now = DateTime.UtcNow;
        // Today: 9 healthy ticks + 1 down tick → day cell "down", uptime 90%.
        for (int i = 0; i < 9; i++) Tick(db, c.Id, now.AddMinutes(-2 - i), 200, 100);
        Tick(db, c.Id, now.AddMinutes(-1), 500, 100);
        await db.SaveChangesAsync();
        ...
        Assert.AreEqual("down", summary.PerService.Single().Uptime30d[^1], "strip cell stays worst-of-day");
        Assert.AreEqual(90.0, summary.Uptime30dPct, 0.01,
            "headline uptime = 9 ok of 10 ticks (90%), not 0 green days of 1 known day (0%)");
    }
