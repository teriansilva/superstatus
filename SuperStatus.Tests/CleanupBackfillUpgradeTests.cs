    ...
    public async Task Cleanup_OnUpgrade_FullyRebackfillsUnreachable_AndAdvancesMarker()
    {
        ...
        try
        {
            ...
            await using (var seed = sp.CreateAsyncScope())
            {
                ...
                {
                    Title = "svc", StatusCheckUrl = "x", ServiceLogoUrl = "",
                    ExpectedStatusCode = 200, Enabled = true, IntervalSeconds = 30,
                ...
                // #418: rollup classifies from FailType (as production stores per tick).
                for (int i = 0; i < 3; i++) db.HistoricalStatusDataSet.Add(new HistoricalStatusData { StatusCheckId = checkId, TimeOfCheckUTC = oldDay.AddMinutes(i), HttpStatusCode = 0, ResponseTimeInMs = 0, CheckFailed = true, FailType = FailType.Unreachable });
                for (int i = 0; i < 2; i++) db.HistoricalStatusDataSet.Add(new HistoricalStatusData { StatusCheckId = checkId, TimeOfCheckUTC = oldDay.AddMinutes(10 + i), HttpStatusCode = 500, ResponseTimeInMs = 80, FailType = FailType.StatusCode });
                for (int i = 0; i < 5; i++) db.HistoricalStatusDataSet.Add(new HistoricalStatusData { StatusCheckId = checkId, TimeOfCheckUTC = oldDay.AddMinutes(20 + i), HttpStatusCode = 200, ResponseTimeInMs = 100, FailType = FailType.NoFail });
