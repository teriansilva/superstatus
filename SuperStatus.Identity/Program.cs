...
//
// #377: the one exception is the hosted public demo instance (PUBLIC_DEMO=true),
// which seeds a well-known administrator so each hourly wipe lands visitors on a
// working login rather than the setup wizard.
...
await SuperStatusIdentityDbInitializer.Seed(
    ...
    seedDemoAdmin: DemoMode.IsEnabledFromEnvironment());
...
void ConfigureServices(WebApplicationBuilder applicationBuilder)
{
    ...
    // #358: self-host dynamic-issuer hardening (host gate + same-origin redirect).
    // No-op unless BOTH IDP_PUBLIC_HTTP and WEBAPP_HTTP are unset — either a pinned
    // issuer (IDP_PUBLIC_HTTP) or reverse-proxy / forwarded-host (WEBAPP_HTTP) keeps it off.
    services.AddDynamicIssuerHardening();

    // #377: resolve PUBLIC_DEMO once at startup. Registered as a concrete singleton so
    // Login.cshtml.cs and _Layout.cshtml take it by injection and tests can supply the
    // off state without touching the process environment.
    services.AddSingleton(DemoModeInfo.FromEnvironment());
