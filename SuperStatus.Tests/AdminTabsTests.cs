...
public class AdminTabsTests
{
    ...
    [TestMethod]
    public void Console_rendersEightTabs_SlasBetweenChecksAndIncidents_defaultsToOverview()
    {
        using var ctx = Ctx();
        var cut = ctx.RenderComponent<Admin>();
        cut.WaitForAssertion(() => Assert.IsTrue(cut.Markup.Contains("Operator console")));

        // #291 Phase B: Webhooks sits between Incidents and Alerts.
        ...
        CollectionAssert.AreEqual(new[] { "Overview", "Checks", "SLAs", "Incidents", "Webhooks", "Alerts", "Settings", "About" }, labels);
    ...
    [TestMethod]
    public void WebhooksPane_ownsListAndLog_AlertsPaneNoLongerHasWebhookLog()
    {
        // #291 Phase B: the webhook list + the execution log live on the Webhooks
        // pane; the Alerts pane keeps delivery config + the alert log only.
        using var ctx = Ctx();
        var cut = ctx.RenderComponent<Admin>();
        cut.WaitForAssertion(() => cut.Find(".console-tabs"));

        var webhooksPane = cut.Find("[data-pane=\"webhooks\"]").InnerHtml;
        Assert.IsTrue(webhooksPane.Contains("Outbound endpoints"), "webhook list panel lives on the Webhooks pane");
        Assert.IsTrue(webhooksPane.Contains("WEBHOOK LOG"), "webhook log relocated to the Webhooks pane");

        var alertsPane = cut.Find("[data-pane=\"alerts\"]").InnerHtml;
        Assert.IsFalse(alertsPane.Contains("WEBHOOK LOG"), "webhook log no longer on the Alerts pane");
        Assert.IsTrue(alertsPane.Contains("ALERT LOG"), "alert log stays on the Alerts pane");

        // Clicking the tab shows the pane.
        cut.FindAll(".console-tab").Single(t => t.TextContent.Trim() == "Webhooks").Click();
        cut.WaitForAssertion(() => Assert.IsFalse(Hidden(cut, "webhooks"), "webhooks pane shown after click"));
        Assert.IsTrue(Hidden(cut, "alerts"), "alerts pane hidden");
    }
