...
namespace SuperStatus.Services.Services
{
    public interface IStatusCheckService
    {
        ...
        Task<List<GridBuildingViewModel>> GetGridBuildings();
    ...
    public class StatusCheckService(IStatusCheckRepository statusCheckRepository, IHistoricalStatusDataRepository historicalStatusDataRepository, IHistoricalStatusActionRepository historicalStatusActionRepository, IWebhookExecutionLogRepository webhookExecutionLogRepository, IDailyStatusRollupRepository dailyStatusRollupRepository, IHttpClientFactory httpClientFactory, ILogger<StatusCheckService> logger, IAutoIncidentCoordinator? autoIncidentCoordinator = null) : IStatusCheckService
    {
        public async Task RefreshDailyRollupsAsync(int daysBack, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var check in checks)
            {
                var rollup = await historicalStatusDataRepository.GetDailyStateRollupAsync(
                    check.Id, check.ExpectedStatusCode, check.ExpectedResponseTimeInMs, since, cancellationToken);
        ...
        {
            // #226: one batched read for the whole dashboard, mirroring GetGridBuildings.
        ...
        public async Task<DashboardSummaryViewModel> GetDashboardSummaryAsync(int incidents30dCount, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var check in checks)
            {
                ...
                // 30-day per-service strip. Worst state of each day wins; days
                ...
                for (int i = 0; i < 30; i++)
                {
                    ...
                    {
                        strip[i] = day.Down > 0 ? "down" : day.Degraded > 0 ? "degraded" : "up";
                        totalDays++;
                        if (strip[i] == "up") totalUpDays++;
                    }
                    else
                    {
                        strip[i] = "gap";
            ...
            double uptime30dPct = totalDays > 0 ? (totalUpDays * 100.0 / totalDays) : 0;
        ...
        /// the persisted <c>DailyStatusRollup</c> table. Summary AND grid both
        /// call this — neither re-invents the cutoff. Two batched queries total.
        ///
        ...
        /// <summary>
        /// Issue #138 (PR-B): sum sample counts over the <paramref name="days"/>
        /// days starting at <paramref name="windowStartDay"/>, returning
        /// (totalSamples, upSamples) where up = total − down − degraded. Drives
        /// the grid's sample-level uptime %, identical to the former
        /// count(NoFail)/count(all) over raw when thresholds are stable.
        /// </summary>
        private static (int Total, int Up) SumWindowSamples(
            Dictionary<DateTime, (int Total, int Down, int Degraded, int Unreachable)> byDay, DateTime windowStartDay, int days)
        {
            int total = 0, upSamples = 0;
            for (int i = 0; i < days; i++)
            {
                if (byDay.TryGetValue(windowStartDay.AddDays(i).Date, out var d))
                {
                    total += d.Total;
                    upSamples += d.Total - d.Down - d.Degraded;
                }
            }
            return (total, upSamples);
        }
        ...
        public static string MostRecentState(StatusCheck check, HistoricalStatusData? mostRecent)
        {
            ...
            if (mostRecent.ResponseTimeInMs > check.ExpectedResponseTimeInMs) return "degraded";
            return "up";
        }
        ...
        public async Task<List<GridBuildingViewModel>> GetGridBuildings()
        {
            // Anonymous endpoint, polled on every Grid page refresh. #138 (PR-B):
            // formerly issued one 30-day raw scan per check (N+1, and incompatible
            // with the short raw-tick retention PR-C introduces). Now derives 30 d
            // / 7 d uptime from the canonical rollup-aware read (old days from the
            // DailyStatusRollup table, recent from raw) and current state from a
            // single batched most-recent query — no full raw scan.
            //
            // CONTRACT CHANGE (Hermes review on #141): the window is now
            // calendar-day-aligned (UTC midnight), NOT the old rolling
            // `now - 30d`/`now - 7d` timestamp cutoff. This is intentional and
            // required: the rollup table is per-UTC-day with no sub-day
            // resolution, so a rolling mid-day cutoff is unrepresentable for old
            // days — and it makes the grid agree with the dashboard summary strip,
            // which already uses this exact calendar window (`now.AddDays(-29).Date`).
            // Effect vs the old path: at 18:00 the trailing edge starts at midnight
            // 29 days ago rather than 18:00 30 days ago (≈ whole-day granularity on
            // the boundary day). Pinned by Grid_Uptime30dWindow_IsCalendarDayAligned.
            DateTime now = DateTime.UtcNow;
            DateTime thirtyStart = now.AddDays(-29).Date;   // 30 calendar days incl. today
            DateTime sevenStart = now.AddDays(-6).Date;     // 7 calendar days incl. today

            IPagedResult<StatusCheck> set = await statusCheckRepository.GetStatusCheckSet();
            var checks = set.Results.ToList();
            var checkIds = checks.Select(c => c.Id).ToList();

            var dailyStateByCheck = await BuildDailyStateByCheckAsync(checkIds, thirtyStart, now, CancellationToken.None);
            var mostRecentByCheck = await historicalStatusDataRepository.GetMostRecentForChecksAsync(checkIds, CancellationToken.None);

            var buildings = new List<GridBuildingViewModel>(checks.Count);
            foreach (StatusCheck sc in checks)
            {
                var byDay = dailyStateByCheck.TryGetValue(sc.Id, out var d)
                    ? d : new Dictionary<DateTime, (int Total, int Down, int Degraded, int Unreachable)>();

                // Sample-level uptime from the daily tallies (up = total − down −
                // degraded). No history => treat as healthy (1.0) so a brand-new
                // check renders pristine rather than wrecked.
                (int tot30, int up30) = SumWindowSamples(byDay, thirtyStart, 30);
                (int tot7, int up7) = SumWindowSamples(byDay, sevenStart, 7);
                double uptime30 = tot30 == 0 ? 1.0 : (double)up30 / tot30;
                double uptime7 = tot7 == 0 ? 1.0 : (double)up7 / tot7;

                mostRecentByCheck.TryGetValue(sc.Id, out var mostRecent);
                int currentFailType = mostRecent != null ? (int)mostRecent.FailType : (int)FailType.NoFail;

                int ageDays = Math.Max(0, (int)(now - sc.Created).TotalDays);

                buildings.Add(new GridBuildingViewModel
                {
                    Id = sc.Id,
                    Title = sc.Title,
                    Seed = sc.Seed,
                    ...
                    Uptime30d = uptime30,
                    Uptime7d = uptime7,
                    CurrentFailType = currentFailType,
                    // #83 persisted running counter — authoritative for trailing
                    // failures and, unlike a raw scan, still correct once raw is
                    // pruned to ~72 h (a long outage would underflow a raw count).
                    ConsecutiveFailures = sc.ConsecutiveFailures,
                    LastCheckUtc = mostRecent?.TimeOfCheckUTC,
                });
            }
        ...
        public async Task<HistoricalStatusAction?> RunPostStatusCheckTasks(StatusCheck statusCheck, HistoricalStatusData historicalStatusData)
        {
            ...
            {
                logger.LogInformation("NOT running task for {Title} because of active throttle!", statusCheck.Title);
                ...
                await SaveLogAsync(new WebhookExecutionLog
                {
                    ...
                    TargetUrl = statusCheck.WebHookOnErrorUrl,
                    HttpStatusCode = 0,
                    ResponseTimeMs = 0,
            ...
            // Issue #77: the outbound webhook call uses the factory's named
            // "status-webhook" client — pooled handler + the shared 10 s
            // timeout configured at registration. A hung target surfaces as
            // TaskCanceledException (handled below as WebhookOutcome.Timeout),
            // bounding how long the scoped DbContext stays pinned. NOT disposed:
            // the factory owns the handler lifetime.
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    await SaveLogAsync(new WebhookExecutionLog
                    {
                        ...
                        Outcome = WebhookOutcome.NonSuccess,
                        ErrorMessage = SanitizeErrorMessage($"HTTP {statusCode} {response.ReasonPhrase}"),
                    });
                    StatusCheckMetrics.WebhooksFired.Add(1, StatusCheckMetrics.OutcomeTag(WebhookOutcome.NonSuccess));
                    return null;
                }
                ...
                StatusCheckMetrics.WebhooksFired.Add(1, StatusCheckMetrics.OutcomeTag(WebhookOutcome.Success));
            ...
            catch (TaskCanceledException tce)
            {
                ...
                StatusCheckMetrics.WebhooksFired.Add(1, StatusCheckMetrics.OutcomeTag(WebhookOutcome.Timeout));
                return null;
            }
            catch (Exception ex)
            {
                ...
                StatusCheckMetrics.WebhooksFired.Add(1, StatusCheckMetrics.OutcomeTag(WebhookOutcome.TransportFailure));
                return null;
            }
        }
        ...
        public async Task<StatusCheck> AddOrUpdateStatusCheck(StatusCheckViewModelBase statusCheck)
        {
            ...
            var newStatusCheck = new StatusCheck
            {
                ...
                // Grid renderer uses these two; they must be set at creation
                // so a newly added check has a stable visual identity from
                // the very first paint. Seed is bounded to a positive int
                // to keep the JS PRNG seeding simple.
