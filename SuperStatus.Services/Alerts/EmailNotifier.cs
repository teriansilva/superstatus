...
public interface IEmailNotifier
{
    ...
    /// when SMTP isn't configured or there are no recipients.</summary>
    Task<EmailSendResult> SendAlertAsync(StatusCheck check, AlertTrigger trigger, CancellationToken cancellationToken = default);
    ...
    ILogger<MailKitEmailNotifier> logger) : IEmailNotifier
{
    ...
    public async Task<EmailSendResult> SendAlertAsync(StatusCheck check, AlertTrigger trigger, CancellationToken cancellationToken = default)
    {
        ...
        var recipients = ParseRecipients(
            string.IsNullOrWhiteSpace(check.EmailRecipients) ? settings.AlertDefaultRecipients : check.EmailRecipients);
