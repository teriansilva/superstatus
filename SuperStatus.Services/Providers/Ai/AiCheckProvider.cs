...
namespace SuperStatus.Services.Providers.Ai
{
    ...
    public sealed class AiCheckProvider : ICheckProvider
    {
        ...
        private static readonly ProviderDescriptor _descriptor = new(
            ...
            // #342: batch add pastes one endpoint base URL per line into "baseUrl"; the
            // shared model / prompt / expected-content / key are chosen once for the batch.
            batchTargetField: AiCheckConfig.BaseUrlKey);
        ...
        public async Task<ProbeResult> ProbeAsync(ProbeContext context, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    // The status code is ours to state; the response body is not relayed (#433).
                    return new ProbeResult
                    {
                        ...
                        PublicMessage = $"the endpoint returned HTTP {(int)resp.StatusCode}",
                    };
                }
                ...
                return new ProbeResult
                {
                    ...
                    Message = message,
                    // #433: Classify's strings are synthesised here from the config and our own
                    // measurements — never from the endpoint's body and never from an exception —
                    // so they are safe to show an operator over the test API.
