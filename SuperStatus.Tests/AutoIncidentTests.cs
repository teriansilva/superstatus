...
public class AutoIncidentTests
{
    ...
    // #415: the FK on Incident.SourceStatusCheckId requires the referenced check to
    // exist. Seed checks 1..upToId so tests that reference a check by literal id have
    // a valid referent (a fresh in-memory DB assigns sequential ids from 1).
    private static async Task SeedChecks(SuperStatusDb db, int upToId)
    {
        for (int i = 1; i <= upToId; i++)
            db.StatusCheckSet.Add(new StatusCheck
            {
                Title = $"c{i}", StatusCheckUrl = "https://x/health", ServiceLogoUrl = "",
                Enabled = true, Created = DateTime.UtcNow,
            });
